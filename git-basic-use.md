...
如果临时需要切换到其他分支，但又不想提交现在的修改，可以使用`git stash`命令将当前的状态保存为一个快照，然后切换到其他分支。等其他的分支的工作完成之后，再切换到原来的分支，执行`git stash pop`取出之前保存的快照，继续原来的工作。
...
首先在本机A先生成一对公钥和私钥，这里简称为`pubkey`和`prikey`，将`pubkey`通过其他渠道（密码访问）添加到要访问的机器B的`~/.ssh/authorized_keys`文件中，此时A便有了一条访问B的通道。

通信原理如下：

首先，当A试图访问B时，B会给A发送一小段使用A的公钥加密后的信息，假设信息原文为`1234`，A接收到信息后，会用自己的私钥解密这段信息，将解密的信息再次发给B，如果解密后的信息一致，则认为A可以访问B，无需密码。
